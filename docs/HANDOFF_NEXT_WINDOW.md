...
README, PUBLICATION, KNOWN-ISSUES, AGENTS and the historical private-layout/data/pipeline/security documents now state their applicable scope. `PUBLIC_SECURITY_PRIVACY_STATUS.md` summarizes the public/private boundary and explicitly lists work requiring a new independent security and privacy audit. Documentation-only changes are in the isolated `.wuwa-description-sync-20260923` clone; do not claim they are published until a later verified push. The already pushed beta.6 source commit is `d201dc3`; last observed GitHub main CI run `35864160516` was in progress. A local beta.6 tag and portable ZIP exist, but no tag push or beta.6 Release was verified. Automatic approval review then rejected a CI status query because the approval usage limit was reached; it did not judge the query unsafe. Do not bypass that approval check with a different network tool. Resume remote verification/release only when approval is available, then verify the tag, CI, assets and SHA-256 independently. The original private repository, its uncommitted changes and protected files remain untouched.
