...
所有任务文件默认保存在项目的 `download/<任务 ID>/` 中。API Base URL、主模型和代理由后端原子保存并在下次启动时恢复；输入新 API Key 时可选仅本次程序会话有效，或安全保存到 Windows Credential Manager。API 只返回“已配置”状态而不回显原文；Key 绝不写入 JSON、localStorage、任务文件或日志。

> [!WARNING]
> 这是公开 Beta 测试版，不是“无需复核”的稳定版。英语流程可用于测试；日语和韩语仍属实验性功能。重要字幕发布前请人工复核，尤其是人名、数字、否定和上下文相关句子。
...
- 完成后在任务顶部预览中文字幕并下载；默认审校列表聚焦需要人工处理的强风险，交付门禁保持不变。
...
3. 浏览器打开后填写视频链接、API Base URL、API Key 和主模型；普通配置可复用，新 Key 可选本次会话或安全保存。下载完成即可开始翻译，也可以上传已有 SRT。
...
5. 任务完成后先预览中文字幕；有必审风险时先审校，清零后再下载可交付字幕。
...
- `final.zh.srt`：自动生成的字幕；没有人工修改且必审风险已清零时可作为交付版。仍有必审项时仅供预览，不应直接发布。
